        ...
        def simulate(self, simulator) -> TSimEvent:
            ...
            def send_rr_op(unit: op_class, op: alu_ops, op_a: int, op_b: int, op_c: int = None, *, result_reg = 0, result_reg_valid = True, fetch_av = False, inst_len = inst_len_16):
                ...
                if not is_cancelled():
                    ...
                    self.sideband_state.tpc = next_tpc
                    self.sideband_state.spc = next_spc
            ...
            yield from send_bubble()
            yield from send_bubble()
            yield from send_mult_op(1,2)
            yield from send_alu_op(alu_ops.a_plus_b, 0x1000, 0x1000, fetch_av=True)
            yield from send_ldst_op(ldst_ops.load, op_a = 4, op_b = 8, op_c = 16, mem_access_len = access_len_32)
            yield from send_mult_op(41,43) # Should get cancelled
            yield from send_bubble()
            yield from send_bubble()
            yield from send_bubble()
            yield from send_mult_op(1,2)
            yield from send_alu_op(alu_ops.a_plus_b, 0x2000, 0x2000, fetch_av=True)
            yield from send_ldst_op(ldst_ops.store, op_a = 4, op_b = 8, op_c = 16, mem_access_len = access_len_32)
            yield from send_mult_op(41,43) # Should get cancelled
            yield from send_bubble()
            yield from send_bubble()
            yield from send_bubble()
